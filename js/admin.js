const AdminApp = {
  ...
  async renderContentTable(table, titleKeys) {
    ...
    this.main().innerHTML = `
      ...
      <table class="admin-table"><thead><tr><th>Title</th><th>Featured</th><th>Created</th><th>Actions</th></tr></thead>
      <tbody>${data.map(row => `
        <tr>
  ...
  async render_categories() {
    ...
    this.main().innerHTML = `
      ...
      <table class="admin-table"><thead><tr><th>#</th><th>Label</th><th>Enabled</th><th>On Homepage</th><th>In ⋯ Menu</th><th>Needs login</th><th></th></tr></thead>
      <tbody>${data.map((c,i) => `
        <tr>
          <td>${i+1} ${i>0?`<button class="btn-icon" onclick="AdminApp.moveCategory('${c.id}',-1)">↑</button>`:''} ${i<data.length-1?`<button class="btn-icon" onclick="AdminApp.moveCategory('${c.id}',1)">↓</button>`:''}</td>
  ...
  async render_homepage() {
    ...
    this.main().innerHTML = `
      ...
      <tbody>${data.map((s,i) => `
        <tr>
          <td>${i+1} ${i>0?`<button class="btn-icon" onclick="AdminApp.moveSection('${s.id}',-1)">↑</button>`:''} ${i<data.length-1?`<button class="btn-icon" onclick="AdminApp.moveSection('${s.id}',1)">↓</button>`:''}</td>
