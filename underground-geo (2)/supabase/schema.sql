...
-- Moderation: hide content without deleting it (used by report handling)
alter table releases add column if not exists hidden boolean default false;
...
-- Hidden content must not be publicly readable — replace the public select policies
drop policy if exists "releases publicly readable" on releases;
create policy "releases publicly readable" on releases for select using (hidden = false or is_admin() or auth.uid() = artist_id);
drop policy if exists "beats publicly readable" on beats;
create policy "beats publicly readable" on beats for select using (hidden = false or is_admin() or auth.uid() = producer_id);
drop policy if exists "posts publicly readable" on posts;
create policy "posts publicly readable" on posts for select using (hidden = false or is_admin() or auth.uid() = author_id);
